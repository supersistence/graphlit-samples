    ...
    if st.session_state['token'] is None:
        ...
        if user_prompt := st.chat_input("Ask me anything about your content.", key="chat_input"):
            ...
            try:
                ...
                logging.debug(f"message: {message}, graph: {graph}, citations: {citations}, error_message: {error_message}")
                ...
                logging.error(f"ValueError encountered: {e}", exc_info=True)
                st.error("An error occurred while processing your request. Please try again later.")
                
            except Exception as e:
                # Catch-all for other unexpected exceptions
                logging.error(f"Unexpected error encountered: {e}", exc_info=True)
